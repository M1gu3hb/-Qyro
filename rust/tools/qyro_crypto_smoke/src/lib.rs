...
mod tests {
    ...
    fn every_outcome_has_a_distinct_stable_code() {
        // A runner reads these numbers out of an exit status. Two steps sharing
        // a code would make a failing log ambiguous.
        let outcomes = [
            SmokeOutcome::Success,
    ...
    fn the_harness_exports_nothing_but_the_smoke_entry_point() {
        ...
        // returned bytes would have to say who owns them, and for key material
        // the answer is that they must not cross at all.
